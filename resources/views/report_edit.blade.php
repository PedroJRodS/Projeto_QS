    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="bg-white dark:bg-[#3E3E3A] shadow-md rounded-lg p-8">
        ...
        <form action="{{ route('reports.update', $report->id) }}" method="POST" class="space-y-5">
          ...
          <div>
            <label for="report_date" class="block text-white text-sm font-medium mb-1">Data do relato<span
                class="text-red-500">*</span></label>
          ...
          <div>
            <label for="reporter_name" class="block text-white text-sm font-medium mb-1">Seu nome<span
                class="text-red-500">*</span></label>
          ...
          <div>
            <label for="condition_id" class="block text-white text-sm font-medium mb-1">Condição<span
                class="text-red-500">*</span></label>
            <select name="condition_id" id="condition_id"
              class="w-full rounded-md border-gray-300 dark:border-gray-700 p-2 shadow-sm">
              <option value="" disabled>Escolha a condição</option>
              @foreach ($conditions as $condition)
              <option value="{{ $condition->id }}" @selected(old('condition_id', $report->condition_id) ==
                $condition->id)>
                {{ $condition->name }}
              </option>
              @endforeach
        ...
        <form action="{{ route('reports.destroy', ['report' => $report->id]) }}" method="post">
          ...
          <div>
            <button class="mt-3 px-6 py-2 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-md transition"
